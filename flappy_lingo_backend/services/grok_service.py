...
def _generate_with_grok(prompt: str) -> str:
    ...
    try:
        ...
        if len(body) > 500:
            body = body[:500] + "..."
        raise RuntimeError(f"xAI HTTP {response.status_code}: {body}") from exc
...
async def generate_question(category: str = "mixed") -> dict:
    target_category = category if category != "mixed" else random.choice(CATEGORIES)
    ...
    try:
        ...
        print(f"[xAI] generate_question failed, using fallback: {exc}")
...
async def generate_vocabulary(topic: str, difficulty: str) -> dict:
    ...
    try:
        ...
        print(f"[xAI] generate_vocabulary failed, using fallback: {exc}")
